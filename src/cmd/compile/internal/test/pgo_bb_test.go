...
var testInline1DumpPatterns = []*checkPair{
	...
	{regexp.MustCompile(`[1-9][0-9]* \..*AS tc\(1\).*bb_test.go:95`), false},
	{regexp.MustCompile(`[1-9][0-9]* \..*AS tc\(1\).*bb_test.go:96`), false},
	{regexp.MustCompile(`[1-9][0-9]* \..*AS tc\(1\).*bb_test.go:97`), false},
	// Check, that assign in second branch has zero counter
	{regexp.MustCompile(`0* \..*SUB.*bb_test.go:100`), false},
}

var testInline3DumpPatterns = []*checkPair{
	// AST checks
	{regexp.MustCompile(`[1-9][0-9]* \..*INLMARK.*bb_test.go:148`), false},
	{regexp.MustCompile(`[1-9][0-9]* \..*INLMARK.*bb_test.go:151`), false},
}
...
func checkInline3Dump(t *testing.T, out []byte, funcName string) {
	// For this case we must check and compare the particular counter values
	type checkPair struct {
		r *regexp.Regexp // Pattern to check the dump
		c int            // counter of this line
	}

	var res = []*checkPair{
		{regexp.MustCompile(`(\d*) \..*INLMARK.*bb_test.go:148`), 0},
		{regexp.MustCompile(`(\d*) \..*INLMARK.*bb_test.go:151`), 0},
	}
	var err error
	...
	for scanner.Scan() {
		...
		for _, pair := range res {
			m := pair.r.FindStringSubmatch(line)
			if m == nil {
				continue
			}

			pair.c, err = strconv.Atoi(m[1])
			if err != nil {
				t.Logf("%s", string(out))
				t.Fatalf("Error in reading test result\n")
			}
		}
	}

	if res[0].c < res[1].c || res[0].c == 0 || res[1].c == 0 {
		t.Logf("%s", string(out))
		t.Fatalf("Error evaluating counters for function %s: counter on line 148 (%d) should be greater than counter on line 151 (%d)\n", funcName, res[0].c, res[1].c)
	}
}
...
func init() {
	...
	if runtime.GOARCH == "amd64" {
		testInline1DumpPatterns = append(testInline1DumpPatterns, &checkPair{regexp.MustCompile(`b.* \([1-9][0-9]*\).*JMP`), false})
		testInline1DumpPatterns = append(testInline1DumpPatterns, &checkPair{regexp.MustCompile(`b.* \(0\).*JMP`), false})
	} else if runtime.GOARCH == "arm64" {
		testInline1DumpPatterns = append(testInline1DumpPatterns, &checkPair{regexp.MustCompile(`b.* \([1-9][0-9]*\).*JMP`), false})
		testInline1DumpPatterns = append(testInline1DumpPatterns, &checkPair{regexp.MustCompile(`b.* \(0\).*JMP`), false})
	}
...
func checkBBPGODumps(t *testing.T, out []byte, regExprs []*checkPair, funcName string) {
	...
	if funcName == "testInline3" {
		checkInline3Dump(t, out, funcName)
	}
}
...
func TestPGOBasicBlocks(t *testing.T) {
	...
	out = buildBBPGOInliningTest(t, dir, pprof, "testInline3")
	checkBBPGODumps(t, out, testInline3DumpPatterns, "testInline3")
}
