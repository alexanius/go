...
func buildssa(fn *ir.Func, worker int) *ssa.Func {
	...
	s.startBlock(s.f.Entry, s.lastCounter)
	...
	// fallthrough to exit
	if s.curBlock != nil {
		s.pushLine(fn.Endlineno, s.curBlock.Counter)
		s.exit()
		s.popLine()
	}
...
type state struct {
	...
	// the last profile counter processed
	lastCounter int64
...
func (s *state) pushLine(line src.XPos, c int64) {
	if !line.IsKnown() {
...
func (s *state) stmt(n ir.Node) {
	...
	case ir.OLABEL:
		...
		if s.curBlock != nil {
			b := s.endBlock()
			b.AddEdgeToP(lab.target, n.Counter())
		}
	...
	case ir.OGOTO:
		...
		b.AddEdgeToP(lab.target, /*n.Counter()*/0) // TODO wft???
	...
	case ir.OIF:
		...
		if len(n.Body) != 0 {
			c := n.Body[0].Counter()
			s.startBlock(bThen, c)
			s.stmtList(n.Body)
			if b := s.endBlock(); b != nil {
				b.AddEdgeToP(bEnd, 0) // TODO wtf??
			}
		}
		if len(n.Else) != 0 {
			c := n.Else[0].Counter()
			s.startBlock(bElse, c)
			s.stmtList(n.Else)
			if b := s.endBlock(); b != nil {
				b.AddEdgeToP(bEnd, 0) // TODO wtf???
			}
...
//		b.Counter = s.lastCounter
	...
	case ir.OJUMPTABLE:
		...
		b.Kind = ssa.BlockIf
		b.SetControl(cmp)
		b.AddEdgeToP(jt, 0)             // in range - use jump table
		b.AddEdgeToP(bEnd, 0)           // out of range - no case in the jump table will trigger
...
func (s *state) ternary(cond, x, y *ssa.Value) *ssa.Value {
	...
	s.startBlock(bThen, s.lastCounter)
	s.vars[ternaryVar] = x
	s.endBlock().AddEdgeTo(bEnd)

	s.startBlock(bElse, s.lastCounter)
	s.vars[ternaryVar] = y
	s.endBlock().AddEdgeTo(bEnd)

	s.startBlock(bEnd, s.lastCounter)
...
func (s *state) assignWhichMayOverlap(left ir.Node, right *ssa.Value, deref bool, skip skipMask, mayOverlap bool) {
	...
	if s.canSSA(left) {
		...
		if left.Op() == ir.OINDEX && left.(*ir.IndexExpr).X.Type().IsArray() {
			...
			s.pushLine(left.Pos(), s.lastCounter)
			defer s.popLine()
...
func (s *state) openDeferExit() {
	...
	s.startBlock(deferExit, s.lastCounter)
	...
	for i := len(s.openDefers) - 1; i >= 0; i-- {
		...
		s.startBlock(bCond, s.lastCounter)
...
func (s *state) boundsCheck(idx, len *ssa.Value, kind ssa.BoundsKind, bounded bool) *ssa.Value {
	...
	s.startBlock(bPanic, s.lastCounter)
	if Arch.LinkArch.Family == sys.Wasm {
...
func (s *state) dottype1(pos src.XPos, src, dst *types.Type, iface, source, target, targetItab *ssa.Value, commaok bool, descriptor *obj.LSym) (res, resok *ssa.Value) {
	...
	if dst.IsInterface() {
		if dst.IsEmptyInterface() {
			...
			b.AddEdgeToP(bOk, 0)
			b.AddEdgeToP(bFail, 0)

			if !commaok {
				// On failure, panic by calling panicnildottype.
				s.startBlock(bFail, s.lastCounter)
				s.rtcall(ir.Syms.Panicnildottype, false, nil, target)

				// On success, return (perhaps modified) input interface.
				s.startBlock(bOk, s.lastCounter)
		...
		b.SetControl(cond)
		b.Likely = ssa.BranchLikely
		b.AddEdgeToP(bNonNil, 0)
		b.AddEdgeToP(bNil, 0)

		s.startBlock(bNil, s.lastCounter)
		if commaok {
			s.vars[typVar] = itab // which will be nil
			b := s.endBlock()
			b.AddEdgeToP(bMerge, 0)
		} else {
		...
		s.startBlock(bNonNil, s.lastCounter)
		typ := itab
		if !src.IsEmptyInterface() {
		...
		if descriptor != nil {
			...
			if base.Flag.N == 0 && rtabi.UseInterfaceSwitchCache(Arch.LinkArch.Name) {
				...
				// At loop head, get pointer to the cache entry.
				//   e := &cache.Entries[hash&mask]
				s.startBlock(loopHead, s.lastCounter)
				idx := s.newValue2(and, typs.Uintptr, s.variable(hashVar, typs.Uintptr), mask)
				...
				b.Kind = ssa.BlockIf
				b.SetControl(cmp1)
				b.AddEdgeToP(cacheHit, 0)
				b.AddEdgeToP(loopBody, 0)

				// Look for an empty entry, the tombstone for this hash table.
				//   if e.Typ == nil { goto miss }
				s.startBlock(loopBody, s.lastCounter)
				cmp2 := s.newValue2(ssa.OpEqPtr, typs.Bool, eTyp, s.constNil(typs.BytePtr))
				b = s.endBlock()
				b.Kind = ssa.BlockIf
				b.SetControl(cmp2)
				b.AddEdgeToP(cacheMiss, 0)
				b.AddEdgeToP(loopHead, 0)

				// On a hit, load the data fields of the cache entry.
				//   Itab = e.Itab
				s.startBlock(cacheHit, s.lastCounter)
				...
				// On a miss, call into the runtime to get the answer.
				s.startBlock(cacheMiss, s.lastCounter)
			}
		}
		...
		s.startBlock(bMerge, s.lastCounter)
...
func (s *state) extendIndex(idx, len *ssa.Value, kind ssa.BoundsKind, bounded bool) *ssa.Value {
	...
	if size > s.config.PtrSize {
		...
		b.SetControl(cmp)
		b.Likely = ssa.BranchLikely
		b.AddEdgeToP(bNext, 0)
		b.AddEdgeToP(bPanic, 0)
