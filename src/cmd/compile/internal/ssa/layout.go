		...
		for _, e := range Bb.BB.Succs {
			if Bb.BB != e.b {
				...
				if /*e.EdgeFreq.RawCount*/ GetCounter(E.f, Bb.BB) != 0 {
					...
					OutCount := uint64(GetCounter(E.f, E.AllBlocks[e.b.LayoutIndex].BB))
					...
					Bb.OutJumps = append(Bb.OutJumps, Jump{B: &E.AllBlocks[e.b.LayoutIndex], V: OutCount})
					if E.f.pass.debug > 2 {
